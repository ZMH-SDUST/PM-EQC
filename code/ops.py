...
def prepare_region_proposals(
    ...
    for res, hs, sz in zip(results, hidden_states, image_sizes):
        ...
        # Clamp boxes to image, 约束坐标框的位置信息
        bx[:, :2].clamp_(min=0)
        bx[:, 2].clamp_(max=sz[1])
        bx[:, 3].clamp_(max=sz[0])

        keep = torch.nonzero(sc >= box_score_thresh).squeeze(1)  # 只保留score（类别概率）大于阈值的box
        ...
        # 约束human box或object box的数量在min_instances和max之间
...
def associate_with_ground_truth(boxes, paired_inds, targets, num_classes, thresh=0.5):
    ...
    for bx, p_inds, target in zip(boxes, paired_inds, targets):
        is_match = torch.zeros(len(p_inds), num_classes, device=bx.device)  # [query_number, verb_class]

        bx_h, bx_o = bx[p_inds].unbind(1)
        gt_bx_h = recover_boxes(target["boxes_h"], target["size"])  # 恢复为左上角和右下角的真实坐标
        gt_bx_o = recover_boxes(target["boxes_o"], target["size"])

        a = torch.min(
            box_ops.box_iou(bx_h, gt_bx_h),  # [42, 2]
            box_ops.box_iou(bx_o, gt_bx_o)  # [42, 2]
        )
        b = a >= thresh
        x, y = torch.nonzero(torch.min(
            box_ops.box_iou(bx_h, gt_bx_h),  # [42, 2]
            box_ops.box_iou(bx_o, gt_bx_o)  # [42, 2]
        ) >= thresh).unbind(1)  # human object pair中，human 和object 均和targetIOU大于阈值
        is_match[x, target["labels"][y]] = 1

        labels.append(is_match)  # 第i个对是HOI，并将相应类别处置为1
...
def compute_prior_scores(  # 每个pair中，human和object的多分类得分：object所有潜在action的得分都是score[pair]
        x: Tensor, y: Tensor,
    ...
    prior_o = torch.zeros_like(prior_h)  # 每个pair都对应一个prior_h 和 prior_o

    s_h = scores[x]  # pair中的human score
    s_o = scores[y]  # pair中的object score

    # Map object class index to target class index
    # Object class index to target class index is a one-to-many mapping
    target_cls_idx = [obj_cls_to_tgt_cls[obj.item()]
                      for obj in labels[y]]  # pair中object所潜在对应的动词集合
    ...
    # 第pair_idx个pair中，human、object属于flat_target_idx类别的先验得分 =  score[pair_idx]
